                ...
                for i in range(length):
                    if float(data[i][0])==0:
                        ...
                        message = Mail(
                            ...
                            html_content=f"Dear {session['username']},"+"check your inventory  account"+"below mentioned quantity not availabe  "+"for more information click the below link  "+"http://localhost:5000/mail")
                        try:
...
def mail():
    ...
    cur.execute("SELECT `productname` from product where avlqty='0.0'AND username=%s" ,(session['username'],))
    data=cur.fetchall()
